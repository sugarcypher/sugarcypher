    ...
    (set, get) => ({
      ...
      requestDataDeletion: async (dataTypes?: string[], reason?: string) => {
        try {
          ...
          get().addSecurityAlert({
            ...
            message: `Your data deletion request has been submitted. Request ID: ${(response as any).requestId || (response as any).deletionId}`,
            dismissed: false,
            actionRequired: false
          });
          
          return (response as any).requestId || (response as any).deletionId || 'unknown';
      ...
      generatePrivacyReport: async () => {
        try {
          ...
          const report = await trpcClient.security.privacy.generateReport.mutate({
            userId: session.userId,
            reportType: 'privacy_summary'
          });
